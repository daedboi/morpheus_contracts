...
contract Zap is Ownable, IZap {
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external override {
        ...
        //check for any left-over tokens when creating LP, and if there are any, send back to user instead of retaining the difference
        uint bal0 = IERC20(pair.token0()).balanceOf(address(this));
        if(bal0>0){
            IERC20(pair.token0()).safeTransfer(msg.sender, bal0);
        }
        uint bal1 = IERC20(pair.token1()).balanceOf(address(this));
        if(bal1>0){
            IERC20(pair.token1()).safeTransfer(msg.sender, bal1);
        }
    }

...
interface FeeDistribution
{
	...
	function claim() external returns (uint256[10] memory _amounts);
}
...
// File contracts/PerpetualEscrowTokenExtension.sol
...
contract PerpetualEscrowTokenExtension is Ownable, ReentrancyGuard, DelayedActionGuard
{
	using SafeERC20 for IERC20;
	...
	constructor(address _escrowToken, address _distributor, uint256 _tokenCount, address _recipient, address _treasury)
	{
	...
	{
		PerpetualEscrowToken(escrowToken)._call(distributor, abi.encodeWithSelector(FeeDistribution.claim.selector));
		for (uint256 _i = 0; _i < tokenCount; _i++) {
			address _token = FeeDistribution(distributor).tokens(_i);
			if (_token == address(0)) break;
			uint256 _balance = IERC20(_token).balanceOf(escrowToken);
			if (_balance > 0) {
				PerpetualEscrowToken(escrowToken)._call(_token, abi.encodeWithSelector(IERC20.approve.selector, address(this), _balance));
				IERC20(_token).safeTransferFrom(escrowToken, recipient, _balance);
			}
		}
	}

	function setDistributorAndTokenCount(address _newDistributor, uint256 _newTokenCount) external onlyOwner delayed
	{
		require(_newDistributor != address(0), "invalid address");
		address _oldDistributor = distributor;
		uint256 _oldTokenCount = tokenCount;
		distributor = _newDistributor;
		tokenCount = _newTokenCount;
		emit UpdateDistributor(_oldDistributor, _newDistributor);
		emit UpdateTokenCount(_oldTokenCount, _newTokenCount);
	}
	...
	event UpdateTokenCount(uint256 _oldTokenCount, uint256 _newTokenCount);
...
// File contracts/PerpetualEscrowTokenFactory.sol
...
contract PerpetualEscrowTokenFactory is ReentrancyGuard
{
	using SafeERC20 for IERC20;

	function computePerpetualEscrowTokenAddress(string calldata _name, string calldata _symbol, bytes32 _salt) public view returns (address _escrowToken)
	{
		bytes32 _hash = keccak256(abi.encodePacked(type(PerpetualEscrowToken).creationCode, abi.encode(_name, _symbol)));
		return Create2.computeAddress(_salt, _hash);
	}

	function createPerpetualEscrowToken(string calldata _name, string calldata _symbol, bytes32 _salt) external nonReentrant returns (address _escrowToken)
	{
		address _address = computePerpetualEscrowTokenAddress(_name, _symbol, _salt);
		IERC20($.LQDR).safeTransferFrom(msg.sender, _address, 1);
		_escrowToken = address(new PerpetualEscrowToken{salt: _salt}(_name, _symbol));
		PerpetualEscrowToken(_escrowToken).transferOwnership(msg.sender);
		emit CreatePerpetualEscrowToken(_escrowToken);
		return _escrowToken;
	}

	event CreatePerpetualEscrowToken(address _escrowToken);
}
...
// File contracts/PerpetualEscrowTokenRateProvider.sol


pragma solidity 0.8.9;

contract PerpetualEscrowTokenRateProvider is IRateProvider
{
	address public immutable escrowToken;
	...
	{
		uint256 _totalReserve = PerpetualEscrowToken(escrowToken).totalReserve();
		uint256 _totalSupply = PerpetualEscrowToken(escrowToken).totalSupply();
		return _totalReserve * 1e18 / _totalSupply;
	}
}

...
abstract contract LinearPool is BasePool, IGeneralPool, IRateProvider {
    ...
    function _scalingFactor(IERC20 token) internal view virtual override returns (uint256) {
        if (token == _mainToken) {
            ...
            // increases in value.
            // In our use case for mLQDR, our wrapped token's exchange rate will be permanently 1.
            return _scalingFactorWrappedToken;
        } else if (token == this) {
    ...
    function _scalingFactors() internal view virtual override returns (uint256[] memory) {
        ...
        // value.
        // In our use case for mLQDR, our wrapped token's scaling factor will be permanently 1.
        scalingFactors[_mainIndex] = _scalingFactorMainToken;
        scalingFactors[_wrappedIndex] = _scalingFactorWrappedToken;
